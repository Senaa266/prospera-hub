...
function InviteBox({ circle, token, onInvited }) {
  ...
  const send = async (event) => {
    ...
    try {
      ...
      if (result.invite?.link && result.invite.email) {
        try {
    ...
    <div className="mt-4 border-t border-ink/10 pt-4">
      <form className="flex flex-wrap gap-2" onSubmit={send}>
        <input
          ...
          className="min-w-0 flex-1 rounded-xl border border-line bg-white px-3 py-2 text-sm text-ink focus:border-ink-strong"
        />
...
function SavingsDetail() {
  ...
  const stats = [
    ...
    },
    {
      icon: 'target',
      label: 'Your position',
      value: circle.myPosition || '—',
      cap: isMember ? `out of ${circle.members} slots` : 'Join to track',
      hue: 'green',
    },
  ]
        ...
        <div className="panel transparency-panel">
          ...
          <div className="panel-body">
            ...
            <div className="roster" role="list">
              {(circle.roster || []).map((m) => (
                <div className={`roster-item ${m.paid ? 'paid' : ''}`} key={m.id}>
                  <span className="roster-avatar">{m.name[0]}</span>
                  <span className="roster-name">
                    {m.name}
                    {m.isYou && <em className="not-italic text-muted"> · you</em>}
                    {m.role === 'creator' && <em className="not-italic text-muted"> · creator</em>}
                  </span>
                  {m.paid ? (
            ...
            {isMember && <InviteBox circle={circle} token={token} onInvited={load} />}
          </div>
        </div>
              ...
              {isMember ? (
                ...
                <>
                  ...
                  <p className="panel-sub">
                    {circle.amount} per {circle.displayCycle.toLowerCase()}, with every payment
                    visible to members.
                  </p>
            ...
            <div className="panel-body">
              ...
              {plan.nextRecipient && (
                <p className="m-0 mt-3 flex items-start gap-2 text-sm leading-6 text-muted">
                  ...
                  Next payout is {fmt(plan.nextAmount)} to {plan.nextRecipient}. Members who fall
                  behind move to the back of the queue.
                </p>
              )}
            </div>
          </div>
