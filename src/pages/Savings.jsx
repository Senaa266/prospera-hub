        ...
        <div className="gc-saved-row">
          <span>
            ...
            {joinedView ? (
              ...
              (circle.visibility === 'Public' ? 'Every payment is public' : 'Members only')
            )}
          </span>
...
function Savings() {
  ...
  const stats = [
    ...
    { icon: 'users', label: 'Active circles', value: String(myCircles.length), cap: `${fmt(contributions)} contributed`, hue: 'blue' },
    {
      ...
      cap: account.invested > 0 ? `${fmt(account.invested)} invested` : 'Ready to spend or invest',
      hue: 'yellow',
    },
  ]
  ...
  return (
    <PageShell>
      ...
      <ContributePaymentModal
        ...
        email={user?.email}
        defaultAmount={
          payTarget?.circle?.weekly || payTarget?.goal?.weekly || 200
        }
